...
function BirdHouse(params) {
	...
	function get_request_token(callback) {
		...
		api(url,'POST',params,function(resp){
			if (resp!=false)
			{
				...
				Ti.API.info("fn-get_request_token: callback is check this "+JSON.stringify(callback));
	...
	function api(url, method, params, callback, auth, setUrlParams, setHeader) {
		...
		// user is authorized so execute API
			...
			Ti.API.info("access_token is "+cfg.access_token+" typof: "+typeof(cfg.access_token));
			if (cfg.access_token!='') {
			...
			Ti.API.info('fn-api: accessor is '+JSON.stringify(accessor));
			OAuth.SignatureMethod.sign(message, accessor);

			Ti.API.info("the API request message: " + JSON.stringify(message));

			// if we are getting request tokens, all params have to be set in URL
			if (typeof(setUrlParams)!='undefined' && setUrlParams==true) {
				//finalUrl = OAuth.addToURL('http://api.twitter.com/1/statuses/update.json', 'status=Ger one ar');
				finalUrl = OAuth.addToURL(message.action, message.parameters);
				Ti.API.info("how sdfds");
				
			}
			// for all other requests only custom params need set in the URL
			else {
				finalUrl = OAuth.addToURL(message.action, initparams);
				Ti.API.info("the API request message: ns sdfthis 1 " + message.action + 'action='+ initparams);
			...
			XHR.onload = function() {
				Ti.API.info("The API response was "+XHR.responseText);
				...
				if (typeof(callback)=='function') {
					...
					Ti.API.info("fn-api: no callback set");
				}
			...
			XHR.onerror = function(e) {
				...
				if (e.error=="Unauthorized") {
					Ti.API.info("API request failed because the access token and token secret must be wrong. Error: "+e);

				} else {
					Ti.API.info('The API XHR request has failed! Status='+XHR.readyState+' and var_dump of e is '+var_dump(e));
				}
			...
			if (typeof(setHeader)=='undefined' || setHeader==true) {
				...
				Ti.API.info('fn-api: auth is '+header);
	...
	function send_tweet(params,callback) {
		api('https://api.twitter.com/1/statuses/update.json','POST','status='+params,function(resp){
...
Ti.API.info('shortlink '+shorturl);
	...
	function authorize(callback) {
		if (!authorized) {
			...
			Ti.API.info("fn-authorize: use this callback function: "+JSON.stringify(callback));
			//Ti.API.info('----- Initializing Authorization Sequence -----');
